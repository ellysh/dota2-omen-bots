...
function M.UpdateState()
  ...
  M.GAME_STATE[M.EC_TARGETABLE_PRESENT] =
    NUM[env.ENEMY_CREEP_TARGETABLE_DATA ~= nil]

  if env.ENEMY_CREEP_TARGETABLE_DATA ~= nil then
    M.GAME_STATE[M.EC_TARGETABLE_IS_TOWER_PROTECTED] =
      NUM[algorithms.DoesTowerProtectUnit(
            env.ENEMY_CREEP_TARGETABLE_DATA)]
  end
